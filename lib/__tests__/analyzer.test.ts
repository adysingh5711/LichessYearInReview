...
describe("analyzeGames", () => {
  ...
  it("computes monthlyPerformance", () => {
    ...
    // KNOWN BUG (plan 005): monthly ratingChange only accumulates on wins
    expect(jan.ratingChange).toBe(0); // +8 (game1) + -8 (game2) — both wins
    expect(feb.ratingChange).toBe(8); // only game5 (the win) contributes
  });
  ...
  it("computes openings sorted by count descending", () => {
    ...
    // KNOWN BUG (plan 005): result "*" counts as a loss in openings and does not reset streaks
    expect(stats.openings[0].losses).toBe(1);
  ...
  it("computes headToHead excluding Anonymous", () => {
    ...
    // KNOWN BUG (plan 005): result "*" counts as a loss in head-to-head
    expect(opponent1.losses).toBe(2);
    expect(opponent1.draws).toBe(0);
    expect(opponent1.winRate).toBe(50);
